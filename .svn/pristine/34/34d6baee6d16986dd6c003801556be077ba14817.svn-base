...
<head>
    ...
    <link rel="stylesheet" href="${basePath}assets/Content/searchResult.css" type="text/css" />
    ...
    $(function(){
    	var currentPage = 1, totalPage = 30, totalVideo = 500; 	//当前页数，总页数，总视频数
    	var tipsTimer; 	//回到第一页定时器
    	var tipsPage = 1; 	//超过页数提示返回
        var catgoryId = '${cid}';//当前分类
        var playIndex = 1;
		var time = 0;
        var timer = new Array();
		...
		var loadVideo = function(p,cid,callback){
			...
			$.get(basePath + 'video/moreList',data,function(res){
				$('#video-list').html("");
...
<body>
	<!--顶部-->
	<div class="top clf">
		<div class="title fl">搜索结果：${key}</div>
		<div class="menu fr clf">
			<ul>
				<li class="search">
					<div class="link">
						<a href="#"></a>
