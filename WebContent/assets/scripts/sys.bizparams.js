iv = {
    ...
    bindData : function(code){
		...
		$.get(basePath + 'sys/bizParams/bizParamsNodes', null, function(data) {
	...
	bind:function(data){
		$("#grid").kendoGrid({
			...
			autoBind:false,
	        dataSource: createDataSource(basePath + 'sys/bizParams/bizParamsList',{
	        	KeyCode: { type: "string" },
    	 ...
    	 $.mdlg({
             ...
             onButtonClick: function(sender, modal, index) {
                 ...
                 if (index == 0) {
                     ...
                     $.post(basePath + 'sys/bizParams/update', JSON.stringify(params), function(data) {
                         if (data.result == true){
...
//                if (typeof(key)!='undefined' && typeof(value)!='undefined') {
                if (id) {
                    ...
                    $.post(basePath + 'sys/bizParams/getParam', JSON.stringify(parma), function(data) {
...
//            param['Id'] = id;
            ...
            $.post(basePath + 'sys/bizParams/delete',JSON.stringify(param), function(data) {
                if (data.result) {
                    $.mdlg.alert('提示', data.message);
                    var treeObj=$.fn.zTree.getZTreeObj("tree-category");
        			var nodes=treeObj.getSelectedNodes();
